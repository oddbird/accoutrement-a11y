...
spec = eval(File.read("accoutrement.gemspec"), binding, "accoutrement.gemspec")
...
desc "Run tests and build accoutrement-#{spec.version}.gem"
task :build => [:test, :gem]

desc "Make make the prebuilt gem accoutrement-#{spec.version}.gem public."
...
desc "Push accoutrement-#{spec.version}.gem to the rubygems server"
task :push_gem do
  sh "gem push pkg/accoutrement-#{spec.version}.gem"
end
